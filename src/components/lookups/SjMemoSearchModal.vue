...
import { IconFileText, IconX, IconSearch } from "@tabler/icons-vue";
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/lookups/sj-memo", {
      params: { q: search.value, page: page.value, limit: itemsPerPage.value },
    });
    items.value = res.data.data.items;
    totalItems.value = res.data.data.total;
  } catch (error) {
    console.error("Gagal memuat data SJ MEMO:", error);
  } finally {
  ...
  >
    <v-card class="lookup-card rounded-lg">
      <v-card-title class="bg-primary text-white d-flex align-center pa-2 px-4">
        <IconFileText :size="16" color="white" class="mr-2" />
        <span class="text-subtitle-2 font-weight-bold">CARI DATA SJ MEMO</span>
        <v-spacer></v-spacer>
        <v-btn
          variant="text"
          size="small"
