...
import { IconBuildingBank, IconX, IconSearch } from "@tabler/icons-vue";
...
const headers = [
  { title: "NAMA", key: "Nama", minWidth: "150px" },
  { title: "KODE", key: "Kode", width: "100px" },
  { title: "REKENING", key: "Rekening", minWidth: "150px" },
];
...
const onRowClick = (event: MouseEvent, row: any) => {
  ...
  if (selectedItem) {
    ...
    console.warn("Data akun tidak ditemukan pada baris yang diklik");
  }
};
    ...
    <v-card class="lookup-card rounded-lg">
      ...
      <v-card-text class="pa-3 bg-grey-lighten-4">
        <v-text-field
          v-model="search"
          @input="onSearchInput"
          label="Cari Kode atau Nama Account..."
          variant="outlined"
          density="compact"
          bg-color="white"
          hide-details
