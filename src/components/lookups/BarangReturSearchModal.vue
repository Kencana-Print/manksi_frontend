...
import { IconBoxSeam, IconSearch, IconDatabaseOff } from "@tabler/icons-vue";
...
const emit = defineEmits(["update:modelValue", "selected", "selectedMultiple"]);

const search = ref("");
const items = ref<BarangItem[]>([]);
const isLoading = ref(false);
const checkedKodes = ref<Set<string>>(new Set());
...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      search.value = "";
      currentPage.value = 1;
      checkedKodes.value = new Set();
      fetchData();
    }
  },
);

const selectItem = (item: BarangItem) => {
  if (props.multiSelect) {
    toggleCheck(item.Kode);
    return;
  }
  emit("selected", item);
...
const toggleCheck = (kode: string) => {
  if (checkedKodes.value.has(kode)) checkedKodes.value.delete(kode);
  else checkedKodes.value.add(kode);
};

const checkedCount = computed(() => checkedKodes.value.size);

const confirmMultiSelect = () => {
  const picked = items.value.filter((i) => checkedKodes.value.has(i.Kode));
  emit("selectedMultiple", picked);
  emit("update:modelValue", false);
};
</script>
  ...
  >
    <div class="lookup-card">
      <div class="lookup-header">
        <IconBoxSeam :size="15" :stroke-width="1.7" color="white" />
        <span
          >Cari Barang {{ jenis
          }}{{ multiSelect ? " (Pilih Banyak)" : "" }}</span
        >
        <v-spacer />
      ...
      <div class="lookup-table-wrap">
        ...
        <table v-else class="lookup-table">
          <thead>
            <tr>
              <th v-if="multiSelect" style="width: 32px"></th>
          ...
          <tbody>
            ...
            >
              <td v-if="multiSelect" @click.stop="toggleCheck(item.Kode)">
                <input
                  type="checkbox"
                  :checked="checkedKodes.has(item.Kode)"
                  readonly
                />
              </td>
      ...
      <div v-if="multiSelect" class="ms-footer">
        <span class="ms-count">{{ checkedCount }} dipilih</span>
        <button
