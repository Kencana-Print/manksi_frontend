...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/garmen/dtf/lhk-so-dtf-form/lookup-so-dtf", {
      params: {
    ...
    console.error("Gagal memuat SO DTF Kaosan:", e);
  } finally {
  ...
  >
    <div class="lookup-card">
      <div class="lookup-header">
        ...
        <span>Cari SO DTF/DTG Kaosan</span>
        <v-spacer />
...
<style scoped>
/* Identik dengan SpkSearchModal.vue — disalin persis supaya visual konsisten */
.lookup-card {
  background: white;
  border-radius: 8px;
  overflow: hidden;
