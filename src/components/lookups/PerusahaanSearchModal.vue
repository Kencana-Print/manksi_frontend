...
import { IconBuilding, IconSearch, IconDatabaseOff } from "@tabler/icons-vue";
...
const fetchData = async () => {
  ...
  try {
    ...
    console.error("Gagal memuat Master Perusahaan", error);
  } finally {
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(
    (i) =>
      (i.perush_kode?.toLowerCase() || "").includes(q) ||
      (i.perush_nama?.toLowerCase() || "").includes(q) ||
      (i.perush_alamat?.toLowerCase() || "").includes(q),
  );
});
  ...
  >
    <div class="lookup-card">
      <div class="lookup-header">
        <IconBuilding :size="15" :stroke-width="1.7" color="white" />
        ...
        <table v-else class="lookup-table">
          ...
          <tbody>
            <tr
              ...
              :key="item.perush_kode"
              class="lookup-row"
              @click="selectItem(item)"
            >
              <td class="td-kode">{{ item.perush_kode }}</td>
              <td>{{ item.perush_nama }}</td>
              <td>{{ item.perush_alamat }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<style scoped>
/* Pastekan css lookup standar di sini (sama dengan css di instruksi Anda) */
.lookup-card {
  background: white;
  border-radius: 8px;
  overflow: hidden;
