...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/lookups/minta-bahan", {
      params: {
        q: search.value,
    ...
    console.error("Gagal memuat Permintaan Bahan:", e);
  } finally {
  ...
  >
    <div class="lookup-card">
      <div class="lookup-header">
        ...
        <span>Cari Permintaan Bahan</span>
        <v-spacer />
      ...
      <div class="lookup-search">
        ...
        <input
          ...
          type="text"
          placeholder="Cari nomor permintaan atau SPK..."
          class="search-input"
          autofocus
        />
          ...
          <tbody>
            ...
            >
              ...
              <td>
                <v-chip
                  size="x-small"
                  :color="item.Approve === 'Y' ? 'success' : 'warning'"
                  variant="flat"
                >
                  {{ item.Approve === "Y" ? "ACC" : "WAIT" }}
                </v-chip>
              </td>
            </tr>
          </tbody>
...
<style scoped>
/* Gunakan style CSS yang persis sama dengan SpkSearchModal.vue */
.lookup-card {
  background: white;
  border-radius: 8px;
  overflow: hidden;
