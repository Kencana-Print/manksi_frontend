...
const fetchData = async () => {
  if (!props.jenis || !props.cabang) return; // Cegah error jika parameter kosong

  isLoading.value = true;
  try {
    const res = await api.get("/lookups/barang-garmen", {
      params: {
        q: search.value,
        jenis: props.jenis,
        cabang: props.cabang,
  ...
  >
    <div class="lookup-card">
      ...
      <div class="lookup-header">
        ...
        <span>Cari {{ props.jenis || "Barang" }}</span>
        <span class="lookup-header-sub ml-2"
          >Cabang: {{ props.cabang || "Semua" }}</span
        >
        <v-spacer />
...
<style scoped>
/* Gunakan style CSS yang persis sama dengan modal lookups Anda yang lain */
.lookup-card {
  background: white;
  border-radius: 8px;
  overflow: hidden;
