...
const props = defineProps<{
  modelValue: boolean;
  cabang: string; // Terima parameter cabang dari form utama
}>();
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/lookups/gudang-produksi", {
      params: {
        q: search.value,
        cabang: props.cabang, // Kirim cabang ke backend
    ...
    console.error("Gagal memuat Gudang Produksi:", e);
  } finally {
...
// Jika cabang berubah saat modal tertutup, reset dan load ulang ketika nanti dibuka
watch(
  () => props.cabang,
  () => {
    if (props.modelValue) {
  ...
  >
    <div class="lookup-card">
      ...
      <div class="lookup-header">
        ...
        <span>Cari Gudang Produksi</span>
        <span class="lookup-header-sub ml-2"
          >Cabang: {{ cabang || "Semua" }}</span
        >
        <v-spacer />
