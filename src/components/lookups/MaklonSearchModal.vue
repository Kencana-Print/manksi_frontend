...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/garmen/dtf/lhk-so-dtf-form/lookup-maklon", {
      params: {
    ...
    console.error("Gagal memuat Maklon Barang:", e);
  } finally {
...
<style scoped>
/* Identik dengan SoDtfSearchModal.vue — disalin persis supaya visual konsisten */
.lookup-card {
  background: white;
  border-radius: 8px;
  overflow: hidden;
