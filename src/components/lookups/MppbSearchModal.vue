...
import { IconPackage, IconX, IconSearch } from "@tabler/icons-vue";
...
const fetchData = async () => {
  ...
  try {
    const res = await api.get("/lookups/mppb", {
      params: { q: search.value, page: page.value, limit: itemsPerPage.value },
    });
    items.value = res.data.data.items;
    totalItems.value = res.data.data.total;
  } catch (error) {
    console.error("Gagal memuat data MPPB:", error);
  } finally {
  ...
  >
    <v-card class="lookup-card rounded-lg">
      <v-card-title class="bg-primary text-white d-flex align-center pa-2 px-4">
        <IconPackage :size="16" color="white" class="mr-2" />
        <span class="text-subtitle-2 font-weight-bold">CARI MPPB</span>
        <v-spacer></v-spacer>
        <v-btn
          variant="text"
          size="small"
