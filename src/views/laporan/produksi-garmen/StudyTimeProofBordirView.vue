...
import { studyTimeProofBordirService } from "@/services/laporan/produksi-garmen/studyTimeProofBordirService";
...
const fetchData = async () => {
  ...
  try {
    const res = await studyTimeProofBordirService.getBrowse(
      dtAwal.value,
      dtAkhir.value,
      cab.value,
    );
...
// data langsung dari browse yang sudah GROUP BY di backend (tanpa
// temp table / string-concat, jadi tidak kebawa bug "#21S01 Column
// count doesn't match value count" seperti di app desktop). ──
...
const onExport = async () => {
  ...
  try {
    ...
    wb.created = new Date();
    const ws = wb.addWorksheet("Study Time Proof Bordir");

    ws.mergeCells(1, 1, 1, 8);
    ws.getCell(1, 1).value = "LAPORAN STUDY TIME PROSES PROOF BORDIR";
    ...
    ws.getCell(4, 7).value = "BORDIR";
    ws.getCell(5, 7).value = "MENIT/PC";
    ws.getCell(5, 8).value = "P/O/J";
    ...
    for (const r of dataRows) {
      ...
      // Jenis/Warna Bahan dst selalu tampil, apapun status grup
      ws.getCell(jRow, 5).value = r.jenisKain ?? "";
      ws.getCell(jRow, 6).value = r.warnaKain ?? "";
      ws.getCell(jRow, 7).value = r.menitPerPc ?? 0;
      ws.getCell(jRow, 8).value = r.pcsPerJam ?? "";
    ...
    saveAs(
      ...
      `Study_Time_Proof_Bordir_${dtAwal.value}_${dtAkhir.value}.xlsx`,
    );
  } catch {
    ...
    <div class="mkb-wrap">
      ...
      <div class="table-scroll">
        <table class="mkb-table">
          ...
          <tbody>
            <tr v-if="isLoading">
              <td colspan="8" class="tc py-4 text-grey">Memuat data...</td>
            </tr>
            <tr v-else-if="!rows.length">
              <td colspan="8" class="tc py-4 text-grey">Tidak ada data.</td>
            </tr>
