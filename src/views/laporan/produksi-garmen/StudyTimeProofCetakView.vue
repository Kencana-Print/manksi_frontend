...
import { studyTimeProofCetakService } from "@/services/laporan/produksi-garmen/studyTimeProofCetakService";
...
const fetchData = async () => {
  ...
  try {
    const res = await studyTimeProofCetakService.getBrowse(
      dtAwal.value,
      dtAkhir.value,
      cab.value,
    );
...
// data langsung dari browse flat per-komponen (tanpa temp table /
// string-concat, jadi tidak kebawa bug "#21S01 Column count doesn't
// match value count" seperti di app desktop). ──
...
const onExport = async () => {
  ...
  try {
    ...
    wb.created = new Date();
    const ws = wb.addWorksheet("Study Time Proof Cetak");

    ws.mergeCells(1, 1, 1, 9);
    ws.getCell(1, 1).value = "LAPORAN STUDY TIME PROSES PROOF CETAK";
    ...
    mergeSingle(7, "NAMA KOMPONEN");
    ws.mergeCells(4, 8, 4, 9);
    ws.getCell(4, 8).value = "CETAK";
    ...
    for (const r of dataRows) {
      ...
      // Nama Komponen dst selalu tampil, apapun status grup
      ws.getCell(jRow, 7).value = r.komponen ?? "";
    ...
    saveAs(
      ...
      `Study_Time_Proof_Cetak_${dtAwal.value}_${dtAkhir.value}.xlsx`,
    );
  } catch {
        ...
        <table class="mkb-table">
          <thead>
            <tr>
              ...
              <th rowspan="2" style="min-width: 160px">NAMA KOMPONEN</th>
          ...
          <tbody>
            ...
            <tr v-for="(r, i) in rows" :key="`${r.spk}-${r.pfdKode}-${i}`">
              <td class="tc">{{ i + 1 }}</td>
