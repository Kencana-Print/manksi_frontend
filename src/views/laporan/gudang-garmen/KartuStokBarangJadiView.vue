...
import { kartuStokBarangJadiService } from "@/services/laporan/gudang-garmen/kartuStokBarangJadiService";
...
const { items, isLoading, canExport, fetchData } = useBrowse({
  menuId,
  fetchApi: async () => {
    if (!filters.value.kode) return [];
    const res = await kartuStokBarangJadiService.getBrowse(
...
watch(
  ...
  () => {
    if (filters.value.kode) fetchData();
  },
);
...
const onUpdateExpanded = async (val: any[]) => {
  ...
  for (const kode of kodeList.filter((k) => k && !detailData.value[k])) {
    ...
    try {
      const res = await kartuStokBarangJadiService.getDetail(
        kode,
        filters.value.startDate,
        filters.value.endDate,
        filters.value.gudang,
...
// ── Lookup Barang (fetchFn custom, tanpa syarat perush/customer) ──
const showBarangModal = ref(false);
const fetchBarangJadi = async (q: string, page: number, limit: number) => {
  const res = await api.get("/lookups/barang-jadi", {
    params: { q, page, limit },
  });
  return res.data.data;
};
const onBarangSelected = (item: any) => {
  filters.value.kode = item.Kode;
  filters.value.namaBarang = item.Nama;
};
...
const onExportMaster = async () => {
  ...
  try {
    ...
    await exportExcelSingle(
      `Kartu_Stok_Barang_Jadi_${filters.value.kode}.xlsx`,
      "Kartu Stok Barang Jadi",
      columns,
      items.value,
      `KARTU STOK BARANG JADI — ${filters.value.kode}`,
    );
  } catch {
...
const onExportDetail = async () => {
  if (!filters.value.kode) {
    toast.warning("Kode Barang harus diisi.");
    return;
  }
  isExportingDetail.value = true;
  try {
    const res = await kartuStokBarangJadiService.getAllDetail(
      filters.value.startDate,
      filters.value.endDate,
      filters.value.kode,
      filters.value.gudang,
    ...
    await exportExcelSingle(
      `Kartu_Stok_Barang_Jadi_Detail_${filters.value.kode}.xlsx`,
      "Rincian Transaksi",
      columns,
      rows,
      `RINCIAN TRANSAKSI — ${filters.value.kode}`,
    );
  } catch {
...
onMounted(() => {
  // Tidak auto-fetch — Kode Barang wajib dipilih dulu oleh user
});
</script>
...
<template>
  ...
  >
    <template #filter-left>
      ...
      <span class="f-label">Kode <span class="req">*</span></span>
      <div class="picker">
        <input
          type="text"
          :value="filters.kode"
          readonly
          class="f-inp"
          style="width: 110px; cursor: pointer; background: #fff9c4"
          placeholder="Wajib diisi"
          @click="showBarangModal = true"
        />
        <input
          type="text"
          ...
          style="width: 200px"
        />
        <button type="button" class="btn-lkp" @click="showBarangModal = true">
          <IconSearch :size="13" color="#1565c0" />
        </button>
      </div>
    </template>
        ...
        <table v-else class="dtl-table">
          <thead>
            <tr>
              ...
              <th style="width: 110px" class="tr">Stok Akhir</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-if="!detailData[item.Kode]?.length">
              <td colspan="6" class="empty-row">Tidak ada transaksi.</td>
            </tr>
          </tbody>
        </table>
      </div>
