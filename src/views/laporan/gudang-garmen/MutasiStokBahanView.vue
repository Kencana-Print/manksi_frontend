...
import { mutasiStokBahanService } from "@/services/laporan/gudang-garmen/mutasiStokBahanService";
...
const firstDayFixed = "2024-01-01";
const todayString = new Date().toISOString().substring(0, 10);

const filters = ref({
  startDate: firstDayFixed,
  endDate: todayString,
  gudang: authStore.gudangBahan?.kode || "",
  namaGudang: authStore.gudangBahan?.nama || "",
...
const { items, isLoading, canExport, fetchData } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await mutasiStokBahanService.getBrowse(
...
// ── Lookup Gudang (F1 sederhana, tanpa modal — pola inline search) ──
...
const onUpdateExpanded = async (val: any[]) => {
  ...
  for (const kode of kodeList.filter((k) => k && !detailData.value[k])) {
    ...
    try {
      const res = await mutasiStokBahanService.getDetail(
        kode,
        filters.value.startDate,
        filters.value.endDate,
        filters.value.gudang,
...
const onExportMaster = async () => {
  ...
  try {
    ...
    await exportExcelSingle(
      `Mutasi_Stok_Bahan_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      "Mutasi Stok Bahan",
      columns,
      items.value,
      "LAPORAN MUTASI STOK BAHAN (MASTER)",
    );
  } catch {
...
const onExportDetail = async () => {
  ...
  try {
    const res = await mutasiStokBahanService.getAllDetail(
      filters.value.startDate,
      filters.value.endDate,
      filters.value.gudang,
      filters.value.tampilkanKosong,
    ...
    await exportExcel(
      `Mutasi_Stok_Bahan_Detail_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      [
        {
          sheetName: "Rincian Transaksi",
          columns,
          rows,
          title: "RINCIAN TRANSAKSI MUTASI STOK BAHAN",
        },
      ],
    );
  } catch {
...
<template>
  ...
  >
    ...
    <template #item.PermintaanMaterial="{ item }">{{
      fmtNum(item.PermintaanMaterial)
    }}</template>
