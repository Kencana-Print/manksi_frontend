...
import { spkVsBpbNonPoService } from "@/services/laporan/gudang-garmen/spkVsBpbNonPoService";
import { IconTruckDelivery, IconFileSpreadsheet } from "@tabler/icons-vue";
...
const { items, isLoading, canExport, fetchData } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await spkVsBpbNonPoService.getBrowse(
...
const onUpdateExpanded = async (val: any[]) => {
  ...
  for (const v of val) {
    ...
    try {
      const res = await spkVsBpbNonPoService.getDetail(nomor);
...
const onExportMaster = async () => {
  ...
  try {
    ...
    await exportExcelSingle(
      `SPK_vs_BPB_NonPO_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      "SPK vs BPB Non PO",
      columns,
      items.value,
      "LAPORAN SPK VS BPB NON PO",
    );
  } catch {
...
const onExportDetail = async () => {
  ...
  try {
    const res = await spkVsBpbNonPoService.getAllDetail(
      filters.value.startDate,
      filters.value.endDate,
    );
    ...
    await exportExcelSingle(
      `SPK_vs_BPB_NonPO_Detail_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      "Detail BPB Non PO",
      columns,
      combinedRows,
      `Rincian SPK vs BPB Non PO  |  Periode: ${formatTanggal(filters.value.startDate)} s.d ${formatTanggal(filters.value.endDate)}`,
    );
    ...
    <template #detail="{ item }">
      <div class="detail-wrap">
        ...
        <table v-else class="dtl-table">
          <thead>
            <tr>
              <th style="width: 130px">Nomor BPB</th>
          ...
          <tbody>
            <tr v-for="(d, i) in detailData[item.SpkNomor]" :key="i">
              <td class="font-weight-bold text-primary">{{ d.NomorBpb }}</td>
