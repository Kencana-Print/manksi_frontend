...
import { spkVsBpbService } from "@/services/laporan/gudang-garmen/spkVsBpbService";
...
const { items, isLoading, canExport, fetchData } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await spkVsBpbService.getBrowse(
...
const onUpdateExpanded = async (val: any[]) => {
  ...
  for (const v of val) {
    ...
    try {
      const res = await spkVsBpbService.getDetail(nomor);
...
const onExportMaster = async () => {
  ...
  try {
    ...
    await exportExcelSingle(
      `SPK_vs_BPB_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      "SPK vs BPB",
      columns,
      items.value,
      "LAPORAN SPK VS BPB",
...
const onExportDetail = async () => {
  ...
  try {
    const res = await spkVsBpbService.getAllDetail(
      filters.value.startDate,
      filters.value.endDate,
    );
    ...
    await exportExcelSingle(
      `SPK_vs_BPB_Detail_${filters.value.startDate}_to_${filters.value.endDate}.xlsx`,
      "Detail BPB",
      columns,
      combinedRows,
      `Rincian SPK vs BPB  |  Periode: ${formatTanggal(filters.value.startDate)} s.d ${formatTanggal(filters.value.endDate)}`,
    );
        ...
        <table v-else class="dtl-table">
          ...
          <tbody>
            ...
            <tr v-if="!detailData[item.SpkNomor]?.length">
              <td colspan="6" class="empty-row">Tidak ada rincian BPB.</td>
            </tr>
          </tbody>
        </table>
      </div>
