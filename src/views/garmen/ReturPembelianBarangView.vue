...
import { returPembelianBarangService } from "@/services/garmen/returPembelianBarangService";
...
// Cabang TIDAK dikunci di modul ini (beda dari Koreksi Stok) — selalu
// full-list, cuma DEFAULT-nya ikut cabang user kalau ada, sesuai
// FormCreate: `if frmMenu.CAB<>'' then cbCab.Text:=frmMenu.CAB else 'ALL'`
const cabangOptions = ["P01", "P02", "P03", "P04", "P05", "ALL"];

const jenisOptions = ref<string[]>([]);

onMounted(() => {
  // Replikasi FormCreate .pas — visibilitas jenis per bagian, pola sama
  // persis Retur Barang/Koreksi Stok
  ...
  // Default cabang: ikut cabang user kalau ada & valid, else ALL
...
const {
  ...
  fetchApi: async () => {
    ...
    const res = await returPembelianBarangService.getBrowse(
  ...
  deleteApi: async (nomor: string) => {
    await returPembelianBarangService.deleteData(nomor);
  },
});
...
const onEdit = (item: any) => {
  router.push(
    `/garmen/barang/retur-pembelian/form/${encodeURIComponent(item.Nomor)}`,
  );
};

const onDelete = async (item: any) => {
  try {
    await returPembelianBarangService.deleteData(item.Nomor);
...
const onPrint = () => {
  ...
  window.open(
    `/garmen/barang/retur-pembelian/print/${encodeURIComponent(nomor)}`,
    "_blank",
  );
};
...
const onExport = async () => {
  ...
  await exportExcelSingle(
    `Retur_Pembelian_${filterState.value.jenis}_${filterState.value.startDate}.xlsx`,
    "Retur Pembelian",
    columns,
    rows,
    `Retur Pembelian - ${filterState.value.jenis}`,
  );
...
const onExportDetail = async () => {
  ...
  await exportExcelSingle(
    `Detail_Retur_Pembelian_${filterState.value.jenis}_${filterState.value.startDate}.xlsx`,
    "Detail Retur Pembelian",
    columns,
    rows,
    `Detail Retur Pembelian - ${filterState.value.jenis}`,
  );
...
const submitAjukan = async () => {
  ...
  try {
    await returPembelianBarangService.ajukanPerubahan(actionForm.value);
    ...
    <template #filter-left>
      ...
      <template v-else>
        <span class="filter-label" style="color: #c62828">
          Bagian Anda tidak punya akses ke modul Retur Pembelian.
        </span>
      </template>
    </template>
