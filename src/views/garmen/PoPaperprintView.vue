...
import { poPaperprintService } from "@/services/garmen/poPaperprintService";
...
// --- STATE FILTER --- (⚠️ TANPA dropdown cabang — lock cabang implisit
// di backend berdasarkan user.cabang, tidak ada opsi apapun di UI)
...
const {
  ...
  menuId: "103",
  fetchApi: async () => {
    const res = await poPaperprintService.getBrowse(
...
// ufrmPoPaper.pas (form) & report cetak.
const onAdd = () => {
  router.push({ name: "PoPaperprintFormCreate" });
};

const onEdit = (item: any) => {
  router.push({ name: "PoPaperprintFormEdit", params: { nomor: item.Nomor } });
};

const onDelete = async (item: any) => {
  try {
    await poPaperprintService.deleteData(item.Nomor);
...
const onPrint = () => {
  ...
  window.open(
    `/garmen/po-paperprint/print/${encodeURIComponent(nomor)}`,
    "_blank",
  );
};
...
const onExport = async () => {
  ...
  await exportExcelSingle(
    `PO_Paperprint_${filterState.value.startDate}.xlsx`,
    "PO Paperprint",
    columns,
    rows,
    "PO Paperprint",
...
const onExportDetail = async () => {
  ...
  const columns: ExcelColumn[] = [
    ...
    { header: "Harga", key: "Harga", width: 12, align: "right" },
    { header: "Keterangan", key: "Keterangan", width: 26 },
  ];

  await exportExcelSingle(
    `Detail_PO_Paperprint_${filterState.value.startDate}.xlsx`,
    "Detail PO Paperprint",
    columns,
    rows,
    "Detail PO Paperprint",
  );
