...
import { koreksiStokBarangService } from "@/services/garmen/koreksiStokBarangService";
...
// ⚠️ Cabang-lock: replikasi FormCreate .pas — kalau bagian user BUKAN
// FINANCE/AUDIT/EDP dan punya cabang home, dropdown dikunci ke 1 opsi
// (cabang user sendiri). DIREKSI TIDAK dikecualikan dari lock ini
// (asimetri sengaja dari source asli — beda dari daftar visibility jenis).
const CABANG_UNLOCK_BAGIAN = ["FINANCE", "AUDIT", "EDP"];
const isCabangLocked = computed(() => {
  const bagian = (authStore.user?.bagian || "").toUpperCase();
  const userCab = authStore.user?.cabang || "";
  return !CABANG_UNLOCK_BAGIAN.includes(bagian) && userCab !== "";
});
const cabangOptions = computed(() => {
  if (isCabangLocked.value) {
    return [authStore.user?.cabang || ""];
  }
  return ["P01", "P02", "P03", "P04", "P05", "HO-", "ALL"];
});
...
onMounted(() => {
  ...
  // Cabang default: kalau locked, paksa ke cabang user; kalau tidak,
  // default ALL (sesuai cbCab.ItemIndex:=6 di Delphi)
  filterState.value.cabang = isCabangLocked.value
    ? authStore.user?.cabang || "ALL"
    : "ALL";
});
...
// Mewarnai baris berdasarkan status pengajuan tutup buku (PIN5)
...
const onEdit = (item: any) => {
  router.push(
    `/garmen/barang/koreksi-stok/form/${encodeURIComponent(item.Nomor)}`,
  );
};

const onDelete = async (item: any) => {
  try {
    await koreksiStokBarangService.deleteData(item.Nomor);
...
const onPrint = () => {
  ...
  window.open(
    `/garmen/barang/koreksi-stok/print/${encodeURIComponent(nomor)}`,
    "_blank",
  );
};
...
const onExport = async () => {
  ...
  await exportExcelSingle(
    `Koreksi_Stok_${filterState.value.jenis}_${filterState.value.startDate}.xlsx`,
    "Koreksi Stok",
    columns,
    rows,
    `Koreksi Stok - ${filterState.value.jenis}`,
  );
...
const onExportDetail = async () => {
  ...
  await exportExcelSingle(
    `Detail_Koreksi_Stok_${filterState.value.jenis}_${filterState.value.startDate}.xlsx`,
    "Detail Koreksi Stok",
    columns,
    rows,
    `Detail Koreksi Stok - ${filterState.value.jenis}`,
  );
...
const submitAjukan = async () => {
  ...
  try {
    await koreksiStokBarangService.ajukanPerubahan(actionForm.value);
    ...
    <template #filter-left>
      ...
      <template v-else>
        <span class="filter-label" style="color: #c62828">
          Bagian Anda tidak punya akses ke modul Koreksi Stok.
        </span>
      </template>
    </template>
