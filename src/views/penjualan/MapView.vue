...
const getSatuanDivisi = (divisi?: string) => {
  if (!divisi) return "Pcs";
  const d = String(divisi).toUpperCase();
  if (d.includes("SPANDUK")) return "Mtr";
  if (d.includes("MMT")) return "M²";
  if (d.includes("GARMEN") || d.includes("KAOSAN")) return "Pcs";
  return "Pcs";
};
...
const onExportMap = () => {
  const columns: ExcelColumn[] = [
    ...
    { header: "Ngedit", key: "Ngedit", align: "center" },
    ...
    `MAP_${filterState.value.startDate}_sd_${filterState.value.endDate}`,
    {
      getData: () => {
        ...
        return rawData.map((r: any) => ({
          ...
          QtyPesananDisplay: `${fmtNum(r.QtyPesanan)} ${getSatuanDivisi(r.Divisi)}`,
...
const showBatalApprovalDialog = ref(false);

const openBatalApprovalDialog = () => {
  if (selected.value.length === 0) return;
  approvalItem.value = selected.value[0];
  showBatalApprovalDialog.value = true;
};

const confirmBatalApproval = async () => {
  isLoading.value = true;
  try {
    await mapService.batalApproveCmo(approvalItem.value.Nomor);
    toast.success("Approval berhasil dibatalkan.");
    showBatalApprovalDialog.value = false;
    fetchData();
  } catch (error: any) {
    toast.error(error.response?.data?.message || "Gagal membatalkan approval.");
  } finally {
    isLoading.value = false;
  }
};
...
<template>
  ...
  >
    ...
    <template #extra-actions="{ selected }">
      ...
      <v-menu v-if="selected.length > 0">
        ...
        <v-list density="compact">
          ...
          <v-list-item @click="openBatalApprovalDialog">
            <template #prepend
              ><IconX :size="16" :stroke-width="1.7"
            /></template>
            <v-list-item-title>Batal Approval MAP</v-list-item-title>
          </v-list-item>
          <v-divider></v-divider>
    ...
    <template #item.QtyPesanan="{ item }">
      {{ fmtNum(item.QtyPesanan) }} {{ getSatuanDivisi(item.Divisi) }}
    </template>
    <template #item.Nominal="{ item }">{{ fmtNum(item.Nominal) }}</template>
  ...
  <v-dialog v-model="showBatalApprovalDialog" max-width="380px" persistent>
    <div class="close-dlg">
      <div class="close-dlg-header" style="background: #c62828">
        <IconX size="16" color="white" class="mr-2" />
        Konfirmasi Batal Approval
        <button class="dlg-x" @click="showBatalApprovalDialog = false">
          ✕
        </button>
      </div>
      <div class="close-dlg-body">
        <div class="f-lbl-sm mb-1">Yakin ingin membatalkan approval MAP:</div>
        <div
          style="
            font-size: 13px;
            font-weight: 700;
        ...
        <div
          ...
          "
        >
          MAP ini juga akan dihapus dari Komitmen Kirim MAP jika belum ada
          Rencana yang diisi.
        </div>
      </div>
      <div class="close-dlg-footer">
        ...
        <button
          class="dlg-btn cancel"
          :disabled="isLoading"
          @click="showBatalApprovalDialog = false"
        >
          Batal
        </button>
      </div>
