...
const onGambarError = () => {
    ...
    if (nomor)
        fallbacks.push(
            `/file-gambar/mintaharga/${encodeURIComponent(nomor)}.jpg`,
        );
    if (referensi)
        fallbacks.push(
...
<template>
    ...
    >
            ...
            <v-menu v-if="selected.length > 0">
                ...
                <v-list density="compact">
                    <v-list-item @click="pengajuanPerubahan">
                        ...
                        /></template>
                        <v-list-item-title
                            >Pengajuan Perubahan</v-list-item-title
                        >
                    </v-list-item>
        ...
        <template #item.HargaRiil="{ item }">{{
            fmtNum(item.HargaRiil)
        }}</template>
    ...
    <v-dialog v-model="showPrintDialog" max-width="450px">
        <v-card class="rounded-lg">
            <v-card-title
                class="bg-primary text-white d-flex align-center pa-3"
            >
                <IconPrinter
                    :size="18"
                    :stroke-width="1.7"
    ...
    <v-dialog v-model="showApprovalDialog" max-width="380px" persistent>
        <div class="close-dlg">
            <div class="close-dlg-header" style="background: #2e7d32">
                ...
                <button class="dlg-x" @click="showApprovalDialog = false">
                    ✕
                </button>
            </div>
    ...
    <v-dialog v-model="showCloseDialog" max-width="380px" persistent>
        <div class="close-dlg">
            ...
            >
                ...
                {{
                    closeAction === "Y"
                        ? "Konfirmasi Close Data"
                        : "Konfirmasi Open Data"
                }}
                <button class="dlg-x" @click="showCloseDialog = false">
                    ✕
                </button>
            </div>
            <div class="close-dlg-body">
    ...
    <v-dialog v-model="dialogDesign" max-width="600px" persistent>
        <v-card class="rounded-lg">
            ...
            <v-card-text class="pa-0">
                ...
                >
                    Tidak ada MAP dengan Design Baru yang belum selesai di
                    periode ini.
                </div>
