...
const getLocalUrl = (item: any) => {
  ...
  const cab = item.mspk_cab || item.Cab || "HO-";
  return `${base}/images/${cab}/map/${encodeURIComponent(item.mspk_nomor)}.jpg`;
};
...
const handleFallbackImage = (e: Event, item: any) => {
  ...
  // Jika sudah pernah coba fallback tapi masih gagal (berarti VPS juga gak ada)
  if (img.dataset.fallbackTried === "true") {
    img.style.display = "none";
    img.parentElement?.classList.add("no-img-active");
    item._workingUrl = null; // Tandai tidak ada gambar sama sekali
    return;
  }

  // Coba ambil dari VPS Lama (Delphi)
  img.dataset.fallbackTried = "true";
  const fallback = getVpsUrl(item);
  img.src = fallback;

  // Simpan url yang sukses/fallback agar tombol download tidak nyasar ke localhost
  item._workingUrl = fallback;
};

// Tombol download akan pakai URL lokal jika ada, atau URL VPS jika lokal gagal
const downloadGambar = (item: any) => {
  // _workingUrl diset otomatis oleh fungsi handleFallbackImage jika lokal 404
...
const fetchKatalog = async (isLoadMore = false) => {
  ...
  try {
    const res = await mapFormService.getKatalogCustomer(
      props.custKode,
    ...
    // Tentukan apakah data masih ada di halaman berikutnya
...
// Handler saat list di-scroll ke bawah
const onScroll = (e: Event) => {
  const target = e.target as HTMLElement;
  // Jika user scroll mendekati bawah (toleransi 50px)
  if (target.scrollTop + target.clientHeight >= target.scrollHeight - 50) {
    // Hindari pemanggilan dobel saat sedang loading
    if (!isLoading.value && !isLoadingMore.value && hasMore.value) {
      page.value++;
      fetchKatalog(true); // Load more = true
    }
  }
};
  ...
  <div v-else class="katalog-layout">
      ...
      <div class="k-topbar">
        ...
        <div class="k-profile-box">
          ...
          <div class="p-row">
            ...
            <span class="p-val">{{ totalKatalog }} MAP</span>
          </div>
        </div>
      </div>
      ...
      <div class="k-grid-container" @scroll="onScroll">
        ...
        <div v-else class="k-grid">
          <div
            ...
            :key="item.mspk_nomor"
            class="k-card"
            :class="{ active: selectedItem?.mspk_nomor === item.mspk_nomor }"
          ...
          >
            ...
            <div class="k-card-info">
              <div class="k-c-title">{{ item.mspk_nama }}</div>
              <div class="k-c-sub">
    ...
    <div class="katalog-side">
      <div class="side-header">
        DETAIL MAP <span>(Satu Kolom Ringkasan)</span>
      </div>

      <div v-if="selectedItem" class="side-body">
        <div class="side-box-title">
          RINGKASAN DETAIL MAP - {{ selectedItem.mspk_nomor }}
        </div>

        <div class="side-img-box">
          <img
            :key="'side-' + selectedItem.mspk_nomor"
        ...
        <table class="side-table">
          <tbody>
            <tr>
              ...
              <td class="st-val fw">{{ selectedItem.mspk_nama }}</td>
            </tr>
            <tr>
              <td class="st-lbl">Nomor MAP</td>
              <td class="st-val">{{ selectedItem.mspk_nomor }}</td>
            </tr>
            <tr>
              <td class="st-lbl">Bahan & Gramasi</td>
              <td class="st-val">
                {{ selectedItem.mspk_kain || "-" }} /
                {{ selectedItem.mspk_gramasi || "-" }}
              </td>
            </tr>
            <tr>
              <td class="st-lbl">Jumlah</td>
              <td class="st-val">
                {{ Number(selectedItem.mspk_jumlah).toLocaleString("id-ID") }}
                pcs
              </td>
            </tr>
            <tr>
              ...
              <td class="st-val">
                Rp {{ Number(selectedItem.mspk_harga).toLocaleString("id-ID") }}
              </td>
            </tr>
            <tr>
              <td class="st-lbl">Tanggal Pesanan</td>
        ...
        <button class="side-dl-btn" @click="downloadGambar(selectedItem)">
          <IconDownload :size="16" class="mr-2" /> UNDUH GAMBAR MAP ASLI
        </button>
      </div>
