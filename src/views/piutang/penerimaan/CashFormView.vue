...
import { cashFormService } from "@/services/piutang/penerimaan/cashFormService";
...
const {
  ...
  initialData,
  fetchApi: async () => {
    const res = await cashFormService.getDetail(nomorParam.value);
  ...
  submitApi: async (payload) => {
    ...
    return cashFormService.saveData(body);
  },
  onSuccess: () => {
    toast.success("Penerimaan Cash berhasil disimpan.");
    router.push("/piutang/penerimaan/cash");
  },
});
...
const validateBeforeSave = () => {
  if (Number(formData.value.debet || 0) === 0)
    return toast.warning("Nilai Debet/Cash tidak boleh 0.");
...
<template>
  <BaseForm
    :title="(isEdit ? 'Ubah' : 'Tambah') + ' Penerimaan Cash'"
    menuId="252"
    :icon="IconCash"
    ...
    >
      ...
      <div class="giro-grid-form">
        <div class="desktop-form-section header-section">
          <div class="sec-title">Informasi Cash</div>
          ...
          <div class="f-row">
            <label class="f-lbl">Nilai Cash</label>
            <v-number-input
                ...
                <tr v-for="(row, idx) in formData.dtlCustomer" :key="idx">
                  ...
                  <td>
                    <input
                      type="text"
                      :value="
                        row.alamat ? `${row.alamat}, ${row.kota || ''}` : ''
                      "
                      readonly
                      class="gi gi-ro"
                    />
