...
import { giroFormService } from "@/services/piutang/penerimaan/giroFormService";
...
const initialData = {
  ...
  tanggalLama: "", // Menyimpan tanggal asli untuk validasi tahun Delphi
  tanggalTempo: formatDateLocal(new Date()),
  account: "", // Nomor Rekening Bank
  rekKode: "", // Kode Rekening Internal
  namaAccount: "", // Nama Bank
  debet: 0,
  notes: "",
  dtlCustomer: [] as any[],
  pin_status: "",
...
const {
  ...
  initialData,
  fetchApi: async () => {
    const res = await giroFormService.getDetail(nomorParam.value);
  ...
  submitApi: async (payload) => {
    ...
    return giroFormService.saveData(body);
  },
  onSuccess: () => {
    toast.success("Penerimaan Giro berhasil disimpan.");
    router.push("/piutang/penerimaan/giro");
  },
});
...
const validateCustomerKode = async (idx: number) => {
  ...
  try {
    // Tembak endpoint lookup standar dengan limit 1
    ...
    // Cari data yang benar-benar persis/exact match
    const exact = items.find(
      (c: any) =>
...
const validateBeforeSave = () => {
  if (!formData.value.nomor?.trim())
    return toast.warning("Nomor Giro wajib diisi.");
  if (Number(formData.value.debet || 0) === 0)
    return toast.warning("Nilai Debet/Giro tidak boleh 0.");
...
<template>
  <BaseForm
    :title="(isEdit ? 'Ubah' : 'Tambah') + ' Penerimaan Giro'"
    menuId="251"
    :icon="IconCreditCard"
    ...
    >
      ...
      <div class="giro-grid-form">
        <div class="desktop-form-section header-section">
          <div class="sec-title">Informasi Giro</div>
          ...
          <div class="f-row">
            <label class="f-lbl">Nomor Giro</label>
            <input
              type="text"
...
/* Row Styling with hide-details approach */
.f-row {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 5px; /* Spacing minimal antar field */
  min-height: 26px;
}
.f-lbl {
  width: 90px;
