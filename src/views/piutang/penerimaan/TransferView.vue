...
import { transferService } from "@/services/piutang/penerimaan/transferService";
import { IconBuildingBank, IconFileDescription } from "@tabler/icons-vue";
...
// Set filter default ke awal bulan s.d hari ini
...
const {
  ...
  fetchApi: async () => {
    ...
    const res = await transferService.getBrowseList(payload);
...
const goAdd = () => {
  router.push({ name: "PenerimaanTransferCreate", query: { mode: "new" } });
};
...
const goDelete = async (item: any) => {
  ...
  try {
    await transferService.deleteTransfer(encodeURIComponent(item.Nomor));
...
const pengajuanPerubahan = async () => {
  ...
  const nomorTransfer = selected.value[0].Nomor;

  isLoading.value = true;
  try {
    await transferService.checkPengajuan(nomorTransfer);

    alasanNomor.value = nomorTransfer;
    alasanText.value = "";
...
const submitPengajuan = async () => {
  ...
  isLoading.value = true;
  try {
    await transferService.requestPin5(alasanNomor.value, alasanText.value);
...
<template>
  <BaseBrowse
    ...
    @edit="goEdit"
    @delete="goDelete"
    @export="exportToExcel('Penerimaan_Transfer')"
  >
    ...
    <v-card class="rounded-lg">
      ...
      <v-card-text class="pa-4 pt-4">
        <div class="text-body-2 mb-2 font-weight-medium">
          Nomor Transaksi Transfer: {{ alasanNomor }}
        </div>
        <v-textarea
          v-model="alasanText"
