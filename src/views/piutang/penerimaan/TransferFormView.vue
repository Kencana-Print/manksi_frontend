...
import { transferFormService } from "@/services/piutang/penerimaan/transferFormService";
...
const initialData = {
  nomor: "",
  kodeBayar: "BT", // Default Bank Transfer
  cabang: "",
  namaPerusahaan: "",
...
const {
  ...
  initialData,
  fetchApi: async () => {
    const res = await transferFormService.getDetail(nomorParam.value);
  ...
  submitApi: async (payload) => {
    ...
    return transferFormService.saveData(body);
  },
  onSuccess: () => {
    toast.success("Penerimaan Transfer berhasil disimpan.");
    router.push("/piutang/penerimaan/transfer");
  },
});
...
const validateBeforeSave = () => {
  if (Number(formData.value.debet || 0) === 0)
    return toast.warning("Nilai Transfer tidak boleh 0.");
...
<template>
  <BaseForm
    :title="(isEdit ? 'Ubah' : 'Tambah') + ' Penerimaan Transfer'"
    menuId="253"
    ...
    >
      ...
      <div class="giro-grid-form">
        <div class="desktop-form-section header-section">
          <div class="sec-title">Informasi Transfer</div>
          ...
          <div class="f-row">
            <label class="f-lbl">Nilai Transfer</label>
