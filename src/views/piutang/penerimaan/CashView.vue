...
const {
  ...
  fetchApi: async () => {
    ...
    const res = await cashService.getBrowseList(payload);
...
const goAdd = () => {
  router.push({ name: "PenerimaanCashCreate", query: { mode: "new" } });
};
...
const goDelete = async (item: any) => {
  ...
  try {
    await cashService.deleteCash(encodeURIComponent(item.Nomor));
...
const pengajuanPerubahan = async () => {
  ...
  const nomorCash = selected.value[0].Nomor;

  isLoading.value = true;
  try {
    // 1. Lakukan Pengecekan Tutup Buku ke Backend
    await cashService.checkPengajuan(nomorCash);

    // 2. Jika lolos (belum dilempar error), buka Modal Alasan
    alasanNomor.value = nomorCash;
    alasanText.value = "";
...
const submitPengajuan = async () => {
  ...
  isLoading.value = true;
  try {
    await cashService.requestPin5(alasanNomor.value, alasanText.value);
...
<template>
  <BaseBrowse
    ...
    @edit="goEdit"
    @delete="goDelete"
    @export="exportToExcel('Penerimaan_Cash')"
  >
    ...
    <v-card class="rounded-lg">
      ...
      <v-card-text class="pa-4 pt-4">
        <div class="text-body-2 mb-2 font-weight-medium">
          Nomor Transaksi Cash: {{ alasanNomor }}
        </div>
        <v-textarea
          v-model="alasanText"
