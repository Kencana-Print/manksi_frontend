...
const menuId = "265"; // MENU_ID Approve SPK Ganti Qty & Jenis Kain
...
const headers = [
  { title: "Program", key: "Program", width: "90px" },
  { title: "Transaksi", key: "Transaksi", width: "90px" },
  { title: "Nomor", key: "Nomor", width: "150px" },
  { title: "Tanggal", key: "Tanggal", width: "100px", align: "center" },
  { title: "Keterangan", key: "Keterangan", minWidth: "220px" },
  { title: "AjuanKe", key: "AjuanKe", width: "70px", align: "center" },
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getGantiQtyKainList(filterState.value);
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitGantiQtyKainOtorisasi({
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_Ganti_Qty_Kain')"
  >
    ...
    <template #extra-actions>
      ...
      >
        <template #prepend><IconSwitchHorizontal :size="15" /></template>
        Otorisasi
      </v-btn>
    </template>
  ...
  <v-dialog v-model="showAuthDialog" max-width="480px" persistent>
    <v-card class="rounded-lg">
      <v-card-title class="auth-dialog-head">
        <IconSwitchHorizontal :size="18" color="white" class="mr-2" />
        Otorisasi Ganti Qty & Jenis Kain
      </v-card-title>
