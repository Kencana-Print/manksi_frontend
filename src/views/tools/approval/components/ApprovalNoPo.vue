...
const menuId = "268"; // MENU_ID SO Tanpa Nomor PO
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getNoPoList(filterState.value);
...
const openOtorisasiHeader = (selected: any[]) => {
  if (selected.length === 0) {
    toast.warning("Silahkan pilih baris SO terlebih dahulu.");
    return;
  }
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitNoPoOtorisasi({
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_SO_Tanpa_Nomor_PO')"
  >
    ...
    <template #extra-actions="{ selected }">
      ...
      >
        <template #prepend><IconFileAlert :size="15" /></template>
        Otorisasi
      </v-btn>
    </template>
  ...
  <v-dialog v-model="showAuthDialog" max-width="480px" persistent>
    <v-card class="rounded-lg">
      <v-card-title class="auth-dialog-head">
        <IconFileAlert :size="18" color="white" class="mr-2" />
        Otorisasi SO Tanpa Nomor PO
      </v-card-title>
      ...
      <v-card-text class="pa-4">
        <div class="d-flex flex-column gap-y-2">
          <div class="delphi-blue-box pa-3 rounded border">
            ...
            <div class="d-row">
              <span class="d-lbl">Nama SO</span>
              <input
                type="text"
                :value="authData.NamaSO"
                readonly
                class="d-inp flex-1"
              />
            </div>
