...
const menuId = "266"; // MENU_ID Mutasi Produksi Tanpa Planning
...
const headers = [
  { title: "Nomor MP", key: "Nomor", width: "150px" },
  ...
  { title: "Gudang Asal", key: "GdgAsal", width: "100px" },
  { title: "Gudang Tujuan", key: "GdgTujuan", width: "100px" },
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getMutasiNoPlanList(filterState.value);
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitMutasiNoPlanOtorisasi({
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_Mutasi_Produksi_NoPlan')"
  >
  ...
  <v-dialog v-model="showAuthDialog" max-width="480px" persistent>
    <v-card class="rounded-lg">
      <v-card-title class="auth-dialog-head">
        ...
        Otorisasi Mutasi Produksi Tanpa Planning
      </v-card-title>
      <v-card-text class="pa-4">
        <div class="delphi-blue-box pa-3 rounded border">
          <div class="d-row">
            <span class="d-lbl">Nomor MP</span>
            <input
              type="text"
          ...
          <div class="d-row">
            <span class="d-lbl">Gudang</span>
            <div class="d-flex gap-2 flex-1">
              <input
                type="text"
                :value="authData.GdgAsal"
                readonly
                class="d-inp flex-1 text-center"
              />
              <span style="align-self: center">→</span>
              <input
                type="text"
                :value="authData.GdgTujuan"
                readonly
                class="d-inp flex-1 text-center"
              />
            </div>
          ...
          <div class="d-row">
            <span class="d-lbl">Tanggal MP</span>
            <input
              type="text"
          ...
          <div class="note-box mt-2">
            SPK/jenis mutasi ini belum memiliki jadwal Planning PPIC saat
            transaksi disimpan.
          </div>
