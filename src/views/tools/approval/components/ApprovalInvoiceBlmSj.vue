...
const menuId = "260"; // MENU_ID Invoice Belum Buat SJ
...
// Sesuai dengan urutan kolom di Delphi (TglInvoice sebelum TglMinta)
const headers = [
  { title: "Nomor", key: "Nomor", width: "160px" },
  { title: "TglInvoice", key: "TglInvoice", width: "100px", align: "center" },
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getInvoiceBlmSjList(filterState.value);
...
const openOtorisasiHeader = (selected: any[]) => {
  if (selected.length === 0) {
    toast.warning("Silahkan pilih baris Invoice terlebih dahulu.");
    return;
  }
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitInvoiceBlmSjOtorisasi({
      nomor: authData.value.Nomor, // Sesuai dengan format param service
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_Invoice_Belum_SJ')"
  >
