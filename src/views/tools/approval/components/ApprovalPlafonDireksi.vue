...
const filterState = ref({
  ...
  belumAccSaja: true,
  jenis: "PENDING_DIREKSI", // ← Direksi hanya lihat yang pending direksi
});
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getPlafonDireksiList(filterState.value);
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitPlafonDireksiOtorisasi({
...
const getRowProps = (data: any) => {
  ...
  if (!item?.PlafonAcc || item.PlafonAcc === "PENDING_DIREKSI") {
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_Plafon_Direksi')"
  >
  ...
  <v-dialog v-model="showAuthDialog" max-width="480px" persistent>
    <v-card class="rounded-lg">
      <v-card-title class="auth-dialog-head">
        ...
        Otorisasi Plafon Customer (Direksi)
      </v-card-title>

      <v-card-text class="pa-4">
        <div class="delphi-blue-box pa-3 rounded border">
          <!-- Badge peringatan plafon besar -->
          <div class="plafon-warning mb-3">
            <span class="warning-icon">⚠</span>
            Plafon melebihi Rp 20.000.000 — memerlukan persetujuan Direksi/Owner
          </div>
          ...
          <div class="d-row">
            ...
            <input
              ...
              readonly
              class="d-inp font-weight-bold plafon-value"
              style="width: 160px; text-align: right"
            />
            <span
              class="ml-2 text-caption font-weight-bold text-deep-purple-darken-2"
            >
              &gt; 20 juta
            </span>
          </div>
...
.auth-dialog-head {
  background: #6a1b9a; /* ungu — beda dari manager yang biru */
  color: white;
  display: flex;
  align-items: center;
  padding: 12px 16px;
