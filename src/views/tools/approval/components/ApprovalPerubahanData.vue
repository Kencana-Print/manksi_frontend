...
const menuId = "259"; // MENU_ID Perubahan Data
...
const { items, isLoading, canEdit, fetchData, exportToExcel } = useBrowse({
  menuId,
  fetchApi: async () => {
    const res = await approvalService.getPerubahanDataList(filterState.value);
...
const submitOtorisasi = async () => {
  ...
  try {
    const res = await approvalService.submitPerubahanDataOtorisasi({
...
<template>
  <BaseBrowse
    ...
    @export="exportToExcel('Approval_Perubahan_Data')"
  >
